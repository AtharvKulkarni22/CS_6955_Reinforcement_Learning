...
def run_bandit_simulation(n_arms=10, n_tasks=2000, n_steps=1000, epsilons=[0, 0.01, 0.1, 0.2, 0.3, 0.5]):
    ...
    for task in tqdm(range(n_tasks), desc='Running Bandit Tasks'):
        ...
        for action in range(n_arms):
            reward = np.random.normal(true_action_values[action], 1)
            action_counts[action] += 1
            action_estimates[action] = reward  # Initial estimate equals first reward
            rewards.append(reward)

        # Step 2: Apply UCB1
        for step in range(n_arms, n_steps):
            total_counts = np.sum(action_counts)
            # ucb_values = action_estimates + np.sqrt(2 * np.log(total_counts) / action_counts)
            ucb_values = action_estimates + np.sqrt(np.log(total_counts) / action_counts)
